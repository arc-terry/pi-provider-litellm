...
| `LiteLLM (...): a fallback served "<route>"` | A LiteLLM router fallback (`x-litellm-attempted-fallbacks`) answered the request. `/model/info` does not expose fallbacks, so protocol and model handling follow the requested route's own deployments. When a fallback belongs to a different model family, such as a Responses-native OpenAI route falling back to a Chat-only Kimi deployment, set `model_info.supported_endpoints: ["/v1/chat/completions"]` on the primary. Warned once per route. |
